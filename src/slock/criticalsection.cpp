...
#include "slock/criticalsection.hpp"
...
namespace mofw {
    namespace system {
        ...
        int basic_critical_section_timedout::lock(unsigned int timeout) {
            ...
            if(timeout == UINT_MAX)
                _bReturn = spinlock_acquire(&m_pHandle, portMUX_NO_TIMEOUT);
            else
                _bReturn = spinlock_acquire(&m_pHandle, (int)timeout);
        ...
        int basic_critical_section_nested::lock(unsigned int t) {
            m_iState = portSET_INTERRUPT_MASK_FROM_ISR();
            return ERR_SYSTEM_NO_RETURN;
        }
        //-----------------------------------
        ...
        int basic_critical_section_nested::unlock() {
            portCLEAR_INTERRUPT_MASK_FROM_ISR(m_iState);
            return ERR_SYSTEM_NO_RETURN;
        }
    }
}
