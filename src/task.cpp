...
namespace mofw {
  namespace internal {
    ...
    inline int32_t get_new_uniqid() {
      static mofw::atomic_counter _counter;
      return _counter++;
    }
  }
  //-----------------------------------
  ...
  basic_task::basic_task(std::string strName, basic_task::priority uiPriority,
      unsigned short  usStackDepth) noexcept
        : m_runningMutex(),
          ...
          m_cvMessage() 
          { m_waitSem.lock(); m_qeMessageQueue.create(); }
  //-----------------------------------
  //  deconstrutor
  //-----------------------------------
  ...
  int basic_task::start(int iCore) {
    ...
    if(m_eventGroup.create() != 0) {
      ...
      m_pHandle = xTaskCreateStaticPinnedToCore(
                  &runtaskstub, 
                  m_strName.c_str(),
                  m_usStackDepth,
                  this, 
  ...
  //  get_handle
  //-----------------------------------
  basic_task::native_handle_type basic_task::get_handle() {
    autolock_t autolock(m_runningMutex);
    return m_pHandle;
  }
  ...
  //  wait
  //-----------------------------------
  int basic_task::wait(convar_t& cv, mutex_t& cvl, TickType_t timeOut)  {
      autolock_t autolock(m_runningMutex);
  ...
  //  post_msg
  //-----------------------------------
  void basic_task::post_msg(task_message* msg, unsigned int timeout) {
      automutx_t lock(m_ltMessageQueueLock);
      m_qeMessageQueue.enqueue(msg, timeout);
      m_cvMessage.signal();
  }

  void basic_task::get_message(task_message *msg, TickType_t timeOut) {
      m_ltMessageQueueLock.lock();

      while (m_qeMessageQueue.is_empty())
					wait(m_cvMessage, m_ltMessageQueueLock, timeOut);

      m_qeMessageQueue.dequeue(msg);

      m_ltMessageQueueLock.unlock();
  }
  bool basic_task::have_message() {
    automutx_t lock(m_ltMessageQueueLock);
