...
namespace mn {
  ...
  template <class TRLock, class TWLock> 
  class rw_lock {
  public:
    using read_lock_t = TRLock;
    using write_lock_t = TWLock;
    using this_type = rw_lock<TRLock, TWLock>; 

    virtual ~rw_lock() = default;

    virtual write_lock_t lock_write() const =0;

		virtual read_lock_t lock_read() const =0;
  };
